...
fqkern = cdms2.open(fin + 'q.kernel.CMIP5plev.nc')
ftkern = cdms2.open(fin + 't.kernel.CMIP5plev.nc')
...
dLW_q_TOA=np.ma.sum(q_LW_kernel_TOA*dq*pdiff, axis=1)
dSW_q_TOA=np.ma.sum(q_SW_kernel_TOA*dq*pdiff, axis=1)
dLW_q_surf=np.ma.sum(q_LW_kernel_surf*dq*pdiff, axis=1)
dSW_q_surf=np.ma.sum(q_SW_kernel_surf*dq*pdiff, axis=1)

dLW_q_TOA_cs=np.ma.sum(q_LW_kernel_TOA_cs*dq*pdiff, axis=1)
dSW_q_TOA_cs=np.ma.sum(q_SW_kernel_TOA_cs*dq*pdiff, axis=1)
dLW_q_surf_cs=np.ma.sum(q_LW_kernel_surf_cs*dq*pdiff, axis=1)
dSW_q_surf_cs=np.ma.sum(q_SW_kernel_surf_cs*dq*pdiff, axis=1)
...
lw_cld_fbk_TOA_grid = MV2.average(-dLW_cloud_TOA/dts_globalmean, axis=0)
sw_cld_fbk_TOA_grid = MV2.average(dSW_cloud_TOA/dts_globalmean, axis=0)
lw_cld_fbk_surf_grid = MV2.average(-dLW_cloud_surf/dts_globalmean, axis=0)
sw_cld_fbk_surf_grid = MV2.average(dSW_cloud_surf/dts_globalmean, axis=0)
...
m.drawparallels(par, dashes=[1,0.001], labels=[1,0,0,0], linewidth=0.1)
m.drawmeridians(mer, dashes=[1,0.001], linewidth=0.1)
x, y = m(*np.meshgrid(lons, lats))
m.contourf(x, y, lw_cld_fbk_TOA_grid, cmap=plt.cm.RdBu_r, levels=LWlevels)
m.colorbar(label=r'W m$^{{-2}}$ K$^{{-1}}$', format='%3.1f')
ax.set_title('TOA LW cloud feedback')
...
m.drawmeridians(mer, dashes=[1,0.001], linewidth=0.1)
x, y = m(*np.meshgrid(lons, lats))
m.contourf(x, y, sw_cld_fbk_TOA_grid, cmap=plt.cm.RdBu_r, levels=SWlevels)
m.colorbar(label=r'W m$^{{-2}}$ K$^{{-1}}$', format='%3.1f')
ax.set_title('TOA SW cloud feedback')
#plt.savefig(fout + 'cldfbks_map_MPI_ESM_amip.pdf')
#plt.suptitle(model_names[0])
plt.savefig(fout + 'cldfbks_TOA_map_CAM5.pdf')
plt.close()

#PLOT SURFACE FEEDBACKS
fig = plt.figure(figsize=(10,6))
ax = fig.add_subplot(211)
#plt.subplots()
m = Basemap(projection='moll',lon_0=180,resolution='i')
...
m.drawmeridians(mer, dashes=[1,0.001], linewidth=0.1)
x, y = m(*np.meshgrid(lons, lats))
m.contourf(x, y, lw_cld_fbk_surf_grid, cmap=plt.cm.RdBu_r, levels=LWlevels)
m.colorbar(label=r'W m$^{{-2}}$ K$^{{-1}}$', format='%3.1f')
ax.set_title('Surface LW cloud feedback')
...
m.drawmeridians(mer, dashes=[1,0.001], linewidth=0.1)
x, y = m(*np.meshgrid(lons, lats))
m.contourf(x, y, sw_cld_fbk_surf_grid, cmap=plt.cm.RdBu_r, levels=SWlevels)
m.colorbar(label=r'W m$^{{-2}}$ K$^{{-1}}$', format='%3.1f')
ax.set_title('Surface SW cloud feedback')
#plt.savefig(fout + 'cldfbks_map_MPI_ESM_amip.pdf')
#plt.suptitle(model_names[0])
plt.savefig(fout + 'cldfbks_surface_map_CAM5.pdf')
plt.close()
...
m.drawmeridians(mer, dashes=[1,0.001], linewidth=0.1)
x, y = m(*np.meshgrid(lons, lats))
m.contourf(x, y, lw_cld_fbk_TOA_grid - lw_cld_fbk_surf_grid, cmap=plt.cm.RdBu_r, levels=LWlevels)
m.colorbar(label=r'W m$^{{-2}}$ K$^{{-1}}$', format='%3.1f')
ax.set_title('ATM LW cloud feedback')
...
m.drawmeridians(mer, dashes=[1,0.001], linewidth=0.1)
x, y = m(*np.meshgrid(lons, lats))
m.contourf(x, y, sw_cld_fbk_TOA_grid - sw_cld_fbk_surf_grid, cmap=plt.cm.RdBu_r, levels=SWlevels)
m.colorbar(label=r'W m$^{{-2}}$ K$^{{-1}}$', format='%3.1f')
ax.set_title('ATM SW cloud feedback')
#plt.savefig(fout + 'cldfbks_map_MPI_ESM_amip.pdf')
#plt.suptitle(model_names[0])
plt.savefig(fout + 'cldfbks_ATM_map_CAM5.pdf')
plt.close()
